            ...
            wxBORDER_SIMPLE)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewTimeline::OnEditMachineStartFrames, this, XRCID("EditMachineStartFrames"));
...
/**
 * Handle the Machine Start Frame menu option being selected
 * @param event event being handled
 */
void ViewTimeline::OnEditMachineStartFrames(wxCommandEvent &event)
{
    MachineStartDialog dlg(this->GetParent(), GetPicture());
    if(dlg.ShowModal() == wxID_OK)
    {
            //What happens when you press ok should be handled in the dialog box class itself
    }
